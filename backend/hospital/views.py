...
def check_avail(doc, dt, tm):  # check if doctor is available in a given slot
    hr, mn = map(int, tm.split(':'))
    ftm = time(hr, mn, 0)  # create a time object

    appointments = Appointment.objects.filter(status=True, doctor=doc, calldate=dt)

    if ftm < time(9, 0) or ftm > time(17, 0):  # if time is not between 9AM to 5PM, reject
        return False
    if time(13, 0) < ftm < time(14, 0):  # if time is between 1PM to 2PM, reject
        return False
    for appointment in appointments:
        if ftm == appointment.calltime and dt == appointment.calldate:  # if some other appointment has the same slot, reject
            return False
    return True
...
def bookapp_view(request):
    if check_patient(request.user):
        ...
        if request.method == "POST":  # if patient books an appointment
            ...
            if appointmentForm.is_valid():  # if form is valid
                docid = int(appointmentForm.cleaned_data.get('doctor'))  # get doctor id from form
                doc = Doctor.objects.filter(id=docid).first()  # get doctor from form

                call_date = appointmentForm.cleaned_data.get('calldate')
                call_time = appointmentForm.cleaned_data.get('calltime')  # already in HH:MM format

                if check_avail(doc, call_date, call_time):  # check if doctor is available during that slot
                    if timezone.now().date() < call_date:  # check if call date is valid
                        app = Appointment(
                            patient=pat,
                            doctor=doc,
                            description=appointmentForm.cleaned_data.get('description'),
...
#                     accountapproval = Patient.objects.filter(status=True, user_id=request.user.id)
#                     if accountapproval:
#                         return JsonResponse({'status': 'success', 'redirect_url': '/profile_pat.html'})
#                     else:
#                         return JsonResponse({'status': 'waiting_approval', 'redirect_url': '/wait_approval.html'})
#                 else:
#                     return JsonResponse({'status': 'failed', 'message': 'Invalid credentials or not a patient'}, status=403)
#             else:
#                 return JsonResponse({'status': 'failed', 'message': 'Form validation failed'}, status=400)
...
def endappointment_doc_view(request,pk):
    if check_doctor(request.user):
        ...
        for i in ChargesApt.objects.all().filter(Aptinfo=ap):
            yh.medicines =f"{yh.medicines}\n{i.commodity} - {str(i.quantity)}"
        yh.save()
...
def discharge_doc_view(request,pk):
    if check_doctor(request.user):
        ...
        pat=ad.patient
        yh = PatHealth.objects.all().filter(patient=pat).first()
        yh.diseases = f"{yh.diseases}\n{ad.description}"
        for i in Charges.objects.all().filter(Admitinfo=ad):
            yh.medicines =f"{yh.medicines}\n{i.commodity} - {str(i.quantity)}"
        yh.save()
...
def bill_apt_view(request,pk):
    ...
    apt=Appointment.objects.all().filter(id=pk).first()
     ...
     # If appointment is not found, return an appropriate error page
    if apt is None:
...
def report_apt_view(request,pk):
    #get information from database and render in html webpage
    apt=Appointment.objects.all().filter(id=pk).first()
     # If appointment is not found, return an appropriate error message
    if apt is None:
        return HttpResponse("Appointment not found", status=404)
